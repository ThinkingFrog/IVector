...
Эта операция выполняется автоматически при соборке проекта если указать флаг `-DBUILD_INTERFACES`. Этот флаг определяет макрос `BUILD_INTERFACES`, наличие которого сигнализирует о сборке библиотеки. За это автоматическое действие отвечает [этот заголовочный файл](https://github.com/ThinkingFrog/IVector/blob/main/include/Interfacedllexport.h).
...
| Метод: `getNext` | |
|---|---|
| Описание: | Реализует получение вектора, отстоящего на несколько шагов вперёд. Новый уникальный индекс будет записан в переменную `index`, полученную по ссылке, а данные вектора будут записаны в буфферный вектор. В случае, если опреация выводит итератор за границы массива, итератор должен быть помечен не валидным, в `index` должно быть записано максимальное значение. |
...
| Метод: `getPrevious` | |
|---|---|
| Описание: | Реализует получение вектора, отстоящего на несколько шагов назад. Новый уникальный индекс будет записан в переменную `index`, полученную по ссылке, а данные вектора будут записаны в буфферный вектор. В случае, если опреация выводит итератор за границы массива, итератор должен быть помечен не валидным, в `index` должно быть записано максимальное значение. |
...
| Метод: `getBegin` | |
|---|---|
| Описание: | Реализует получение вектора, расположенного в начале множества. Новый уникальный индекс будет записан в переменную `index`, полученную по ссылке, а данные вектора будут записаны в буфферный вектор. |
...
| Метод: `getEnd` | |
|---|---|
| Описание: | Реализует получение вектора, расположенного в конце множества. Новый уникальный индекс будет записан в переменную `index`, полученную по ссылке, а данные вектора будут записаны в буфферный вектор. |
| Параметры: | `vec` - буфферный вектор, куда будут записаны элементы вектора, который запрашивается итератором, <br />`index` - уникальный индекс вектора, передаётся как ссылка на текущий индекс вектора, чтобы после получения ресурса этот индекс изменился на новый, соответствующий полученному вектору. |
| Возвращаемое значение: | Код ошибки. <br />`SUCCESS` в случае успеха. <br />Может вернуть: <br />`SOURCE_SET_EMPTY`, если множество пусто, <br />`SOURCE_SET_DESTROYED`, если итератор запрашивает данные у множества, которое уничтожено. <br />Подробная информация пишется в [логгер](#setlogger). |
